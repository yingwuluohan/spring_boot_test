package com.unisound.iot.common.config;
...
public class MyDailyRollingFileAppender extends FileAppender {
	 ...
	 * 改造过的构造器
	 */
	public MyDailyRollingFileAppender(Layout layout, String filename, int maxBackupIndex) throws IOException {
	...
	private class Log4jHockThread extends Thread {
		@Override
		public void run() {
			QuietWriter qw = MyDailyRollingFileAppender.this.getQw();
			if (qw != null) {
				qw.flush();
			}
		}
	...
	public boolean isAsSevereAsThreshold(Priority priority) {
		...
		// 只判断是否相等，而不判断优先级.可实现将不同级别的日志文件写到不同的文件中
